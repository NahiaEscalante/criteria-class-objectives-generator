...
1. **AuthContext - useNavigate fuera de Router**
   - **Ubicación:** `src/contexts/AuthContext.tsx:27`
   - **Problema:** `useNavigate()` se llama fuera de `<BrowserRouter>`
   - **Impacto:** Puede causar error en runtime
   - **Solución:** Mover la lógica de navegación fuera del contexto o usar `window.location.href`
...
1. **Corto plazo:**
   ...
   - Corregir `useNavigate` en AuthContext
